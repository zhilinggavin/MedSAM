...
data_root = "/media/NAS06/gavinyue/disentanglement/scripts_segmentation/MedSAM/data/npy/CT_RealFIb"
# medsam_lite_checkpoint = args.pretrained_checkpoint
medsam_lite_checkpoint = "/media/NAS06/gavinyue/disentanglement/MedSAM/work_dir/LiteMedSAM/lite_medsam.pth"
...
checkpoint = "/media/NAS06/gavinyue/disentanglement/MedSAM/workdir/LiteMedSAM-ViT-B-20240810-0126/medsam_lite_latest.pth"
makedirs(work_dir, exist_ok=True)
args.task_name = "LiteMedSAM-ViT-B"
if args.use_wandb:
    import wandb
    ...
    wandb.init(
        ...
        resume="allow",  # Ensure that the run is resumed
        id="k93bj5kz"        
    )
...
for epoch in range(start_epoch + 1, num_epochs):
    ...
    print(
        f'Time: {datetime.now().strftime("%Y%m%d-%H%M")}, Epoch: {epoch}, Loss: {epoch_loss_reduced}'
    )
    ...
    if epoch_loss_reduced < best_loss:
        ...
        torch.save(checkpoint, join(model_save_path, f"epoch{epoch}_medsam_lite_best.pth"))
    ...
    epoch_loss = [1e10 for _ in range(len(val_loader))]
    epoch_start_time = time()
    ...
    for step, batch in enumerate(pbar):
        ...
        pbar.set_description(f"Epoch {epoch} at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}, val_loss: {loss.item():.4f}")

    epoch_end_time = time()
    epoch_loss_reduced_val = sum(epoch_loss) / len(epoch_loss)
    val_losses.append(epoch_loss_reduced_val)

    if args.use_wandb:
        wandb.log({
            "epoch_loss": epoch_loss_reduced,
            "epoch_val_loss": epoch_loss_reduced_val,
            "epoch": epoch
        })
    
    # epoch_loss_reduced_val = 1e10
    # %% plot loss
    plt.plot(val_losses)
    plt.title("Dice + Binary Cross Entropy + IoU Loss")
    plt.xlabel("Epoch")
    plt.ylabel("Val_Loss")
    plt.savefig(join(model_save_path, "val_losses.png"))
    plt.close()
